...
class TodoRecycleAdapter : RecyclerView.Adapter<TodoRecycleAdapter.TodoRecycleHolder>() {
    ...
    override fun onBindViewHolder(holder: TodoRecycleHolder, position: Int) {
        holder.bind(my.get(position))
        holder.itemView.setOnClickListener {
            val id= my.get(position).id?.toInt()
            val bundle= bundleOf("id" to id)
            Navigation.findNavController(holder.itemView).navigate(R.id.action_listFragment_to_detailed_todo,bundle);
        }
    }
